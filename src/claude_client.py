...
Engineer's question: {question}"""
    ...
    def create_spec(self, title: str, brief: str, format_example: str = "") -> str:
        """
        Generate a complete spec page from a title and a one-line brief.
        If format_example is provided, Claude mirrors its structure exactly.
        """
        if format_example:
            system = f"""You are SpecBot, helping a product team write new feature specifications.

The team has a specific way of writing specs. Here is a real example — study its structure, headings, tone, and level of detail carefully:

--- EXAMPLE SPEC START ---
{format_example}
--- EXAMPLE SPEC END ---

Your job is to write a new spec that matches this format exactly:
- Use the same headings in the same order
- Match the tone and writing style
- Match the level of detail in each section
- Do not add sections that aren't in the example
...
Rules:
- Work only from the title and brief provided — don't invent requirements.
- Where the brief doesn't give enough detail to fill a section, write "TBD — discuss with team".
- Flag genuine open decisions as open questions.
- Be concise. Match the length and density of the example."""
        else:
            system = """You are SpecBot, helping a product team write new feature specifications.
...
- Where the brief doesn't give enough detail to fill a section, write "TBD — discuss with team".
- Flag genuine open decisions as open questions.
- Be concise. A good spec is one page, not five."""

        prompt = f"""Write a spec page for the following:
